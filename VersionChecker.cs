...
internal static class VersionChecker
{
    ...
    private const string ValheimVersions = "0.218.15,0.218.16";
    #endregion
    ...
    public static bool Check(ManualLogSource logger, PluginInfo info, bool allowWrongVersion = false, ConfigFile? config = null)
    {
        ...
        if (ValheimVersions.Split(',').Contains(currentValheimVersion)) return true;

        var wrongVersionMessage =
            $"The current version of {info.Metadata.Name} (v{info.Metadata.Version}) is designed for Valheim " +
            (ValheimVersions.Split(',').Count() >= 2 ? "versions " + string.Join(" | ", ValheimVersions.Split(',')) : "version " + ValheimVersions) +
